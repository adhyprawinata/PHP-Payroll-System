...
require_once("../config/koneksi.php");
	switch($act){
		default:?>
				...
				?>
				<div>
				<form action="?modul=salary&act=inputSalary" method="POST">
					...
					<input type="submit" value="Input Salary" style="float:right" class="btn btn-primary">
				</form>
				</div>
				<div style="clear:both;content:'';display:block;">
				</div>
				<form method="POST" action="../controller/doSubmitSalary.php">
			...
			<?php
							$i=1;
							if(isset($_GET['bulan'])){
								$query = "SELECT * FROM salary WHERE bulan_tahun = '$bulan'";
							}else{
								$query = "SELECT * FROM salary WHERE bulan_tahun = '$bulan_tahun'";
							}
							...
							?>	
								...
								<tr>
									<td> 
										<?php echo $b['id_user']; ?>
										<input type="hidden" name="id_salary[]" value="<?php echo $b['id_salary']; ?>"/>
									</td>
									<td>
									...
									<td><?php if($b['level'] == 4) echo "<a href=''>Delete</a> | ";?><a href="?modul=salary&act=detailSalary&id=<?php echo $b['id_salary']; ?>">Detail</a>
									</td>
								</tr>
						<?php
								$i++;
			...
			<div style="text-align:center;"><input type="submit" value="Submit" class="btn btn-primary"></div>
			</form>
	<?php
	break;
		case "detailSalary" :
