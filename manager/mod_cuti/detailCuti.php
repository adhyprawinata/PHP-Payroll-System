...
<?php
		...
		<div class="form-group"><label class="col-sm-3">
			Tanggal Diterima</label> <div class="col-sm-3"> :
			<?php 
			if($data['tanggal_mulai_diterima'] != "0000-00-00"){
				echo date('d M', strtotime($data['tanggal_mulai_diterima']))." - ". date('d M, Y', strtotime($data['tanggal_akhir_diterima']));
			}else{
				echo "-";
		...
		<div class="form-group"><label class="col-sm-3">
			Kuota Terpakai</label> <div class="col-sm-3">: <?php 
			if($data['kuota_terpakai'] != 0){
				echo $data['kuota_terpakai']; 
			}else{
				echo "-";
			}
...
<?php
 $sql_message = mysqli_query($k,"SELECT * FROM message WHERE id_cuti = '$id'");
 $data_message = mysqli_fetch_array($sql_message);
 if(mysqli_num_rows($sql_message) > 0){	 
	 echo "<div>$data_message[isi]</div>
			<div>$data_message[tanggal]  $data_message[jam]  by $data_message[author]</div> 
	 ";
 }
  ?>
<?php if($data['level'] == 1){
